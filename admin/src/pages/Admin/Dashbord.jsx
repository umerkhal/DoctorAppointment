      ...
      <div className='flex flex-wrap gap-3'>
        <div className='flex items-center gap-3 bg-white p-4 min-w-52 rounded border-2 border-gray-100 cursor-pointer hover:scale-105 transition-all '>
          ...
          <div>
            ...
            <p className='text-gray-400'>Docyors</p>
          </div>
        </div>
      ...
      <div className='bg-white'>
        ...
        <div className='pt-4 border border-t-0'>
        {
          dashData.latestBooking?.map((item, index)=>(
            <div className='flex items-center px-6 py-6 gap-3 hover:bg-gray-100 border-b' key={index}>
            <img className='rounded-full w-10' src={item.dashData.image} alt="" />
            <div className='flex-1 text-sm'>
              <p className='text-gray-800 font-medium'>{item.dashData.name}</p>
              <p className='text-gray-600'>{slotDateFormat(item.slatDate)}</p>
            </div>
              {
                          item.cancelled 
